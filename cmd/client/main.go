...
func getGRPCConnection(token credentials.PerRPCCredentials, brokerNamespace string) *grpc.ClientConn {
	...
	if err != nil {
		...
		conn, err = grpc.Dial(url, grpc.WithTransportCredentials(creds), grpc.WithPerRPCCredentials(token)) //nolint:staticcheck // Dial is supported throughout grpc 1.x; NewClient changes connection semantics.
	}
